...
from Networksecurity.utils.main_utils.utils1 import write_yaml_file
...
class DataValidation:
    ...
    def datadrift(self,base_df,current_df,threshold=0.05):
        try:
            status = True
            report = {}
            for column in base_df.columns:
                ...
                report.update({column:{
                    "p_value":float(sample_dis.pvalue),
    ...
    def initiate_data_validation(self)-> DataValidationArtifact:
        try :
            ...
            if not status :
                error_message = " Train dataframe doesnt not conatin all columns."
            status = self.validate_number_of_columns(dataframe=test_dataframe)
            if not status :
                error_message = " Test dataframe doesnt not conatin all columns." 
            
            status = self.datadrift(base_df=train_dataframe,current_df=test_dataframe)
